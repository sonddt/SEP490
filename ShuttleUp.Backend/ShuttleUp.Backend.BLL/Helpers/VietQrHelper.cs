...
public static class VietQrHelper
{
    ...
    public static string? ResolveBin(string? paymentBankBin, string? paymentBankName)
    {
        ...
        if (string.IsNullOrWhiteSpace(paymentBankName)) return null;
    ...
    {
        if (string.IsNullOrWhiteSpace(bin) || string.IsNullOrWhiteSpace(accountNumber)) return null;
        ...
        if (!string.IsNullOrWhiteSpace(addInfo)) url += "&addInfo=" + Uri.EscapeDataString(addInfo.Trim());
        return url;
    }
}
